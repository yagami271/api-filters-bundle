...
final class IlikeFilterStrategy implements DbalFilterStrategyInterface
{
    public function getType(): string
    {
        return FilterType::Ilike->value;
    }
    ...
    public function apply(QueryBuilder $queryBuilder, string $column, mixed $value, string $parameterName): void
    {
        if (\is_array($value)) {
            ...
            foreach ($value as $i => $item) {
                ...
                $orConditions[] = \sprintf('LOWER(%s) LIKE LOWER(:%s) ESCAPE \'!\'', $column, $param);
                $queryBuilder->setParameter($param, '%'.$this->escapeWildcards($item).'%');
            }
            $queryBuilder->andWhere($queryBuilder->expr()->or(...$orConditions));
        } else {
            $queryBuilder->andWhere(\sprintf('LOWER(%s) LIKE LOWER(:%s) ESCAPE \'!\'', $column, $parameterName))
